...
namespace Gener		// генерация кода в assembler
{
	...
	string genConditionCode(Lexer::LEX& tables, int i, string& cyclecode)	// оператор условия (if)
	{
		string str;
		conditionnum++;
		cyclecode.clear();
	...
	void CodeGeneration(Lexer::LEX& tables, Parm::PARM& parm, Log::LOG& log)	// генерация
	{
		...
		ofstream ofile("..\\Generation\\Generation\\Debug\\Generation.asm");
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_BRACELET:	// переход на метку в конце критерия
			{
